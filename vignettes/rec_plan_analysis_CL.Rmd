...
This vignette accompanies a manuscript we will post to _PeerJ Preprints_ and submit to _Conservation Letters_. The preprint can be found [here](#).
...
w_plan <- rbind(spp_w_final, spp_w_revision)
...
drafts <- c("Conservation Strategy", "Draft", "Outline")
w_draft <- filter(have, plan_status %in% drafts) %>%
  distinct(title, species, .keep_all = TRUE)
str_c("# species with draft or outline plans: ", dim(w_draft)[1], "\n\n") %>% cat()
```
...
n_official <- lapply(year, function(x) { sum(official$year_plan <= x) }) %>%
  unlist()
n_draft <- lapply(year, function(x) { sum(drafts$year_plan <= x) }) %>%
  unlist()

n_list_df <- data_frame(year, n_listed, n_official, n_draft)
n_list_df$n_no_plan <- with(n_list_df, n_listed - n_official - n_draft)
...
plt <- ggplot(n_list_df, aes(y = n_listed, x = year)) +
  ...
  geom_line(aes(y = n_official, x = year), 
            colour = "gray70",
            linetype = "dashed") +
  geom_line(aes(y = n_draft, x = year), 
            colour = "gray70",
            linetype = "dotted") +
  labs(x = "", y = "Count") +
  theme_hc() + 
...
n_draft_spp <- lapply(
  year, function(x) sum(drafts$year_plan == x, na.rm = TRUE)
) %>% unlist()
n_plans <- lapply(
